...
<label class="input-label w-72">
    <div class="input-group input-group-divider grid-cols-[auto_1fr_auto]">
        ...
        <input class="px-2 w-24" type="search" placeholder="Zoeken..." bind:value={query} on:input={() => onSearch()} >
        <button use:menu={{menu: 'example', interactive: true}} on:click={() => onSearch()}>Zoeken</button>
    </div>
</label>
